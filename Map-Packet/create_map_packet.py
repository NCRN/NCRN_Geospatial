...
## Clear old files from the directions folder
...
#print("Plots not added: ", list_difference_result_directions)

## Create blank docx to go behind map
## Need to run for maps that have an even number of directions sheets (2, 4, etc.)
#for panel in panel_list:
#    folder_path_working = os.path.join(__ROOT_DIR, panel, 'Directions', 'Working')
#    if panel == 'Panel_1':
...
#    create_blank_docx(folder_path_working, map_list)
## Loop over the panel folders to convert Directions docx to pdf
#    print("Converting {0} Directions docx to pdf...".format(panel))
#    folder_path_output = os.path.join(__ROOT_DIR, panel, 'Directions')
#    for filename in os.listdir(folder_path_working):
#        # Full path of the docx
#        in_file = os.path.join(folder_path_working, filename)
#        # Name of the output pdf
#        out_filename = filename.replace('.docx', '.pdf')
#        # Full path of the output pdf
#        out_file = os.path.join(folder_path_output, out_filename)
#        # Create an empty PDF
...
#        if ((panel == 'Panel_1') & (filename.startswith("PRWI 02 PRWI (Park Entrance).pdf"))):
#            pdf_delete_page(folder_path_output, filename, 1)
#        elif ((panel == 'Panel_4') & (filename.startswith("CATO 02 CATO.pdf"))):
#            pdf_delete_page(folder_path_output, filename, 1)
...
for panel in panel_list:
    ...
    folder_path_working = os.path.join(__ROOT_DIR, panel, 'Appendix', 'Working')
    folder_path_output = os.path.join(__ROOT_DIR, panel, 'Appendix')
    #for filename in os.listdir(folder_path_working):
    #    # Full path of the docx
    #    in_file = os.path.join(folder_path_working, filename)
    #    # Name of the output pdf
    ...
    for filename in os.listdir(folder_path_output):
        try:
            pdf_delete_page(folder_path_output, filename, 3)
            # rotate pdfs to landscape and copy back to working folder
            pdf_port_to_land(folder_path_output, folder_path_working)
        except:
            pass
...
for panel in panel_list:
    ...
    ## Set the folder path to the Appendix folder
    folder_appendix = os.path.join(__ROOT_DIR, panel, 'Appendix', 'Working')  
    # Create a list to populate with the Appendix
    pdfiles_appendix = []
    create_pdf_list(folder_appendix, pdfiles_appendix)
